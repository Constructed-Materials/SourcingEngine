...
using SourcingEngine.Search.Lambda;

/// <summary>
/// Local debugging harness that consumes from a local RabbitMQ queue
/// and invokes the Search Lambda handler, enabling full F5 debugging.
///
/// Usage:
///   1. Start local RabbitMQ:  docker compose -f src/SourcingEngine.BomExtraction.Lambda/local/docker-compose.local.yml up -d
///   2. Run this:              dotnet run --project . -- --local
///   3. Publish a message to bom-extraction-result-queue via RabbitMQ Management UI (localhost:15672)
///
...
public static class LocalRunner
{
    ...
    {
        Console.WriteLine("=== SourcingEngine Search Lambda — Local Debug Runner ===");
        Console.WriteLine();
    ...
    /// bom-extraction-result-queue, converting each into a Lambda invocation.
    /// </summary>
    private static async Task ConsumeFromLocalQueue()
    {
        Console.WriteLine("Connecting to local RabbitMQ (localhost:5672)...");
        Console.WriteLine("Publish messages to 'bom-extraction-result-queue' via http://localhost:15672");
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            var mqEvent = new RabbitMQEvent
            {
                ...
                {
                    ["bom-extraction-result-queue::/"] = new List<RabbitMQEvent.RabbitMQMessage>
                    {
                        new()
                        {
                            Data = bodyBase64,
        ...
        await channel.BasicConsumeAsync(
            queue: "bom-extraction-result-queue",
            autoAck: false,
            consumerTag: "",
            noLocal: false,
            exclusive: false,
    ...
    {
        // Broker config — local RabbitMQ, no SSL
        ...
        Environment.SetEnvironmentVariable("Lambda__ResultExchange", "sourcing.engine");
        Environment.SetEnvironmentVariable("Lambda__ResultRoutingKey", "search.result");
        Environment.SetEnvironmentVariable("Lambda__ZeroResultRoutingKey", "search.zero-result");

        // Database — use local/dev connection string if not already set
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("Database__ConnectionString")))
        {
            // Point to the dev Supabase instance (same as appsettings.json in Console project)
            Environment.SetEnvironmentVariable("Database__ConnectionString",
                "Host=aws-1-us-east-1.pooler.supabase.com;Port=5432;Database=postgres;Username=postgres.dtxsieykjcvspzbsrrln;Password=6F2vMzYrxSpQpWzD;SSL Mode=Require;Trust Server Certificate=true");
        }

        // Bedrock settings — use defaults from appsettings.json if not already set
        Environment.SetEnvironmentVariable("Bedrock__Enabled",
...
internal class LocalLambdaContext : ILambdaContext
{
    ...
    public string FunctionName => "sourcing-engine-dotnet-local";
    public string FunctionVersion => "$LATEST";
    public ICognitoIdentity Identity => null!;
    public string InvokedFunctionArn => "arn:aws:lambda:us-east-2:000000000000:function:sourcing-engine-dotnet-local";
    public ILambdaLogger Logger { get; } = new ConsoleLambdaLogger();
    public string LogGroupName => "/aws/lambda/sourcing-engine-dotnet-local";
