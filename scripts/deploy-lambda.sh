#!/usr/bin/env bash
# ============================================================================
# deploy-lambda.sh — Build, push, and deploy the BOM Extraction Lambda
#
# Usage:
#   ./deploy-lambda.sh                    # Build + push image only
#   ./deploy-lambda.sh --deploy           # Build + push + CDK deploy
#   ./deploy-lambda.sh --synth            # CDK synth only (dry run)
#
# Prerequisites:
...
LAMBDA_DIR="$REPO_ROOT/src/SourcingEngine.BomExtraction.Lambda"
CDK_DIR="$REPO_ROOT/infra/BomExtractionLambdaCdk"

# Defaults
AWS_REGION="${AWS_REGION:-us-east-2}"
ECR_REPO_NAME="bom-extraction-lambda"
...
echo "=== BOM Extraction Lambda Deployment ==="
...
if $DEPLOY; then
    ...
    aws lambda update-function-code \
        --function-name bom-extraction-dotnet \
